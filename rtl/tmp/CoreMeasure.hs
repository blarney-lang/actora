...
makeCore debugIn = do
  -- Instruction memory
  instrMem :: RAM InstrPtr Instr <- makeRAMInit "instrs.mif"
  ...
  -- Performance
  finished :: Reg (Bit 1) <- makeReg false
  finishPulse :: Reg (Bit 1) <- makeDReg false
  cycleCount :: Reg (Bit 40) <- makeReg 0
  gcCount :: Reg (Bit 40) <- makeReg 0
  displayVal :: Reg (Bit 40) <- makeReg dontCare
  displayCount :: Reg (Bit 32) <- makeReg dontCare
  always do
    when (finished.val.inv) do
      cycleCount <== cycleCount.val + 1
      when (gcActive.val) do
        gcCount <== gcCount.val + 1
  ...
  let finishCode =
        Seq [
          Do [
            displayVal <== cycleCount.val
          , displayCount <== 10
          ]
        , While (displayCount.val .!=. 0) (
            Seq [
              Wait (debugOut.notFull)
            , let x = (0 :: Bit 4) # range @39 @36 (displayVal.val)
              in  Do [enq debugOut (x .<. 10 ? (48 + x, 55 + x))]
            , Do [displayVal <== displayVal.val .<<. (4 :: Bit 5),
                  displayCount <== displayCount.val - 1]
            ]
          )
        , Do [
            displayVal <== gcCount.val
          , displayCount <== 10
          ]
        , While (displayCount.val .!=. 0) (
            Seq [
  ...
  run (finishPulse.val) finishCode
